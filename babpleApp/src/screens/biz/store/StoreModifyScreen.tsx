...
interface StoreModifyScreenProps {
  visible: boolean;
  onClose: () => void;
}
 ...
 * 마트 정보 수정 화면
 */
const StoreModifyScreen: React.FC<StoreModifyScreenProps> = ({
  visible,
  onClose,
}) => {
  ...
  const loadStoreInfo = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        setBusinessNumber(data.biz_reg_no || '');
        setRepresentative(data.owner || '');
        setStoreIntro(data.description || '');
        setStoreContact(data.phone_number || '');
        setStoreAddress(data.address || '');
        setDetailedAddress(''); // 기존 주소 로드 시 상세 주소 초기화
        setShowDetailedAddressInput(false); // 기존 주소 로드 시 상세 주소 입력창 숨김
        ...
        if (data.operating_hours && typeof data.operating_hours === 'object') {
          const newOpeningTimes: {[key: string]: {hour: number; minute: number; isAM: boolean}} = {};
          const newClosingTimes: {[key: string]: {hour: number; minute: number; isAM: boolean}} = {};
          const days: string[] = [];

          Object.keys(data.operating_hours).forEach(day => {
            const hours = data.operating_hours[day];
            if (hours && hours.opening && hours.closing) {
              const opening = parseTimeString(hours.opening);
              const closing = parseTimeString(hours.closing);
              if (opening && closing) {
                newOpeningTimes[day] = opening;
                newClosingTimes[day] = closing;
                days.push(day);
              }
            }
          });
        ...
        if (data.off_days && Array.isArray(data.off_days)) {
          const parsedHolidays = data.off_days.map((h: any) => ({
            week: h.week || '',
            day: h.day || '',
          }));
      ...
      console.error('❌ [상점 수정] 정보 로드 오류:', error);
  ...
  /**
   * 시간 문자열 파싱 (예: '10:00' -> {hour: 10, minute: 0, isAM: true})
   */
  const parseTimeString = (timeStr: string): {hour: number; minute: number; isAM: boolean} | null => {
  ...
  const handleAddressSelect = (address: string, coordinates?: {latitude: number; longitude: number}) => {
    ...
    setDetailedAddress(''); // 새 주소 선택 시 상세 주소 초기화
    setShowDetailedAddressInput(true); // 새 주소 검색 후 상세 주소 입력창 표시
  ...
  const handleSave = async () => {
    // 필수 정보 검증
    if (!storeName || !businessNumber || !storeAddress) {
      alert('오류', '필수 정보를 모두 입력해주세요. (상점명, 사업자 등록번호, 주소)');
      return;
    }
    ...
    try {
      ...
      const response = await StoreAPI.updateStore({
        ...
        owner: representative || undefined,
      ...
      if (response.success) {
        alert('성공', '상점 정보가 수정되었습니다.').then(() => {
          onClose();
        });
      } else {
        alert('실패', response.message || '상점 정보 수정에 실패했습니다.');
      }
    } catch (error: any) {
      console.error('❌ [상점 수정] 오류:', error);
      alert('오류', error.response?.data?.message || '상점 정보 수정 중 오류가 발생했습니다.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        <View style={styles.container}>
          ...
          <View style={styles.header}>
            ...
            />
            <Text style={styles.headerTitle}>마트 정보 수정</Text>
            <View style={{width: 40}} />
          </View>
            ...
            <View style={styles.imageUploadContainer}>
              <TouchableOpacity
                ...
                {uploadingImage ? (
                  ...
                  <Image
                    source={{
                      uri: profileImageUri || buildImageUrl(profileImageUrl) || '',
                    }}
